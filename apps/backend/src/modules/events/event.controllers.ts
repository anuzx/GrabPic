...
const getAllPhotos = asyncHandler(async (req, res) => {
  ...
  if (photos.length === 0) {
    throw new ApiError(400, "Unable to fetch photos for this event");
  }
...
const removeEvent = asyncHandler(async (req, res) => {
  ...
  // await Promise.all(photos.map(p => cloudinary.uploader.destroy(p.publicId)));
  //
  // also to remove the face embeddings generated by ai service ,can be done by raw query or by calling ai-service for cleanup
...
const searchFace = asyncHandler(async (req, res) => {
  ...
  const { facePhotoUrl } = req.body;
  if (!facePhotoUrl) {
    throw new ApiError(400, "facePhotoUrl is required");
  }

  //This needs a synchronous response member is waiting to see their photos
  const aiResp = await fetch(`${config.aiServiceUrl}/search-face`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ eventId, facePhotoUrl }),
  });
  ...
  //Restore order to match ai-service ranking
